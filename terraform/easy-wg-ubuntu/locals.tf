locals {
  ...
  temp_conf      = templatefile("${path.module}/files/clients.conf", local.client)
  ...
  peers          = formatlist(file("${path.module}/files/peer.conf"), local.client_ips, flatten(data.external.client.*.result.public))
